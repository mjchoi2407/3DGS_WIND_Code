...
from wind3dgs.teacher.resident_preconditioner_reuse import reuse_first_preconditioned_rhs, ReusingResidentGMRES
from wind3dgs.teacher.resident_gmres_parallel_reset import ParallelResetGMRES
...
class ReuseTests(unittest.TestCase):
    def test_restart_rhs_and_zero(self):
        ...
        results=[]
        for optimized in ('original','context','owned','parallel_reset'):
            ...
            factory = {'owned':ReusingResidentGMRES,'parallel_reset':ParallelResetGMRES}.get(optimized,ResidentGMRES)
            solver=factory(A.operator,pre,b,x,tol,restart=3,cycles=20)
            wp.load_module(module=__name__,device='cuda:0')
            from contextlib import nullcontext
            with reuse_first_preconditioned_rhs() if optimized == 'context' else nullcontext():
        ...
        for candidate in results[1:]:
            for i,(old,new) in enumerate(zip(results[0],candidate)):
