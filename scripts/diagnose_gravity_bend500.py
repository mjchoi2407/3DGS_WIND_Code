"""동결1/500의 실패 프레임만 새 출력에서 재현한다. 원본 수정/재개 없음."""
...
a.out.mkdir(parents=True,exist_ok=False);write(a.out/'config.json',{'source':str(r),'source_chunk_sha256':chunk['files'][chunk['path']],'frame':120,'rebuild':a.rebuild,'cycles':a.cycles,'split':a.split,'frames':a.frames,'dt':1/3840/a.split,'precision':'fp64_hilo','scope':'실패 프레임부터 별도 재현;매substep동기화/진단 포함'})
policy=ShellSolvePolicy(**dict(plan['official_policy'],linear_cycles=a.cycles));per_frame=64*a.split;steps=per_frame*a.frames;dt=1/3840/a.split
