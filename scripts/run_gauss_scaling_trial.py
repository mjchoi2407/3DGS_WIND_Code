...
p.add_argument('--mode',choices=['baseline','scaled','mixed'],default='baseline');p.add_argument('--steps',type=int,default=512);p.add_argument('--repeats',type=int,default=3);a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
if a.mode=='scaled':
 from wind3dgs.teacher.resident_gauss_equilibration import EquilibratedGaussStepper as ResidentGaussStepper
if a.mode=='mixed':
 from wind3dgs.teacher.resident_gauss_mixed import MixedGaussStepper as ResidentGaussStepper
...
write('config.json',{'input':str(a.input),'mode':a.mode,'precision':a.precision,'tolerance':a.tolerance,'policy':policy,'steps':a.steps,'dt':1/30720,'duration_s':a.steps/30720,'repeats':a.repeats,'training_eligible':False})
...
try:
 ...
 def count():
  d={'c':s.c.numpy().tolist(),'failure':int(s.failure.numpy()[0])}
  if hasattr(s,'ir_counts'):d['mixed_counts']=s.ir_counts.numpy().tolist()
  return d
 ...
 def failure_snapshot():
  np.savez(a.out/'failure_state.npz',**dict(zip(keys,[x.numpy().reshape(n,3) for x in s.state])),U_hi=s.U.numpy(),U_lo=s.L.numpy(),acc=s.acc.numpy(),norms=s.norms.numpy(),stats=s.stats.numpy())
 if r['warmup_counts']['failure']:
  failure_snapshot();r['status']='solver_failure';write('report.json',r);print(r,flush=True);raise SystemExit(0)
 times=[];counts=[]
 ...
 if any(c['failure'] or c['c'][6]!=a.steps for c in counts):
  failure_snapshot();raise RuntimeError('측정 반복 중 실패')
 ...
 if hasattr(s,'equilibration'):
  eq=s.equilibration;np.savez(a.out/'scaling.npz',row=eq.row.numpy(),col=eq.col.numpy())
